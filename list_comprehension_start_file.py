...
int_list = [len(i) for i in words if i != "the"]
print(int_list)
...
count_num = [i for i in range(1001) if "4" in str(i)]
print(count_num)

## count how many times the word 'the' appears in the text file - 'sometext.txt'
myfile = open("sometext.txt", "r").read().split()
result = len([i for i in myfile if "the" in i])
print(result)

## Extract the numbers from the following phrase ##

phrase = (
    "In 1984 there were 13 instances of a protest with over 1000 people attending. On average there were 15 reported injuries at each "
    "event, with about 3 or 4 that were classifled as serious per event."
)

numbers = [i for i in phrase.split() if i.isdigit()]
print(numbers)
